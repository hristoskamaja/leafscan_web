...
// ── Mock data (врз основа на ER дијаграм) ────────────────────────────────────
// LATER: const res = await plantsAPI.getAll(); setPlants(res.data);
const MOCK_PLANTS = [
...
// ── Detail modal ──────────────────────────────────────────────────────────────
function DetailModal({ plant, onEdit, onClose }) {
      ...
      <div className="modal-box" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-body">
          ...
          <div className="detail-row">
            ...
            <div className="detail-item">
              <div className="detail-label"><MapPin size={13} strokeWidth={1.8} /> Growing Region</div>
          ...
          <div className="detail-section">
            <div className="detail-section-title">
              <FileText size={13} strokeWidth={1.8} /> Description
            </div>
        ...
        <div className="modal-footer">
          ...
          <button className="btn btn--primary" onClick={() => onEdit(plant)}>
            <Pencil size={14} strokeWidth={1.8} /> Edit Plant
          </button>
        </div>
      </div>
    </div>
...
// ── Form modal ────────────────────────────────────────────────────────────────
function FormModal({ plant, onSave, onClose }) {
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" onClick={e => e.stopPropagation()}>
        <div className="modal-header">
          <h3 className="modal-title">{isEdit ? 'Edit Plant' : 'Add New Plant'}</h3>
          <button className="modal-close" onClick={onClose}>
            <X size={18} strokeWidth={2} />
          </button>
        </div>
        <div className="modal-body">
          ...
          <div className="form-field">
            ...
            <textarea className={`form-input form-textarea ${errors.description ? 'form-input--error' : ''}`}
              ...
              placeholder="Describe the plant..." rows={3} />
          ...
          <div className="form-row">
            <div className="form-field">
              ...
              <input className="form-input"
                value={form.growing_season} onChange={e => set('growing_season', e.target.value)}
                placeholder="e.g. Spring - Summer" />
            </div>
            <div className="form-field">
              <label className="form-label">Growing Region</label>
              <input className="form-input"
                value={form.growing_region} onChange={e => set('growing_region', e.target.value)}
          ...
          <div className="form-field">
            ...
            {preview ? (
              <div className="img-preview-wrap">
                ...
                <button className="img-remove" onClick={() => { set('image', null); setPreview(null); }} type="button">
  ...
  return (
    <div className="modal-overlay" onClick={onCancel}>
      <div className="modal-box modal-box--sm" onClick={e => e.stopPropagation()}>
        ...
        <h3 className="modal-warn-title">Delete Plant</h3>
        <p className="modal-warn-desc">
          Are you sure you want to delete <strong>{plant.name}</strong>? This cannot be undone.
        </p>
...
export default function Plants() {
  ...
  const handleSave = data => {
    // LATER: if (data.id) await plantsAPI.update(data.id, data); else await plantsAPI.create(data);
  ...
  const handleDelete = () => {
    // LATER: await plantsAPI.delete(selected.id);
    ...
    <div className="plants-page">
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">{plants.length} plants in the system</p>
        </div>
        <button className="btn btn--primary"
          onClick={() => { setSelected(null); setModal('form'); }}>
          <Plus size={16} strokeWidth={2.2} /> Add Plant
        </button>
      </div>
      ...
      <div className="dis-filters">
        <div className="dis-search">
          ...
          <input className="dis-search-input" placeholder="Search plants..."
          ...
          <table className="dis-table">
            <thead>
              <tr>
                ...
                <th className="sortable" onClick={() => toggleSort('scientific_name')}>Scientific Name <SortIcon col="scientific_name" /></th>
                <th className="sortable" onClick={() => toggleSort('type')}>Type <SortIcon col="type" /></th>
                <th className="sortable" onClick={() => toggleSort('growing_region')}>Region <SortIcon col="growing_region" /></th>
                <th>Actions</th>
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                <tr><td colSpan={5}>
                  <div className="dis-empty">
                    ...
                    <div className="dis-empty-title">No plants found</div>
                  ...
                  <td onClick={e => e.stopPropagation()}>
                    <div className="dis-actions">
                      <button className="dis-action-btn dis-action-btn--edit"
                        onClick={() => { setSelected(p); setModal('form'); }} title="Edit">
                        <Pencil size={15} strokeWidth={1.8} />
                      </button>
                      <button className="dis-action-btn dis-action-btn--delete"
                        onClick={() => { setSelected(p); setModal('delete'); }} title="Delete">
      ...
      {/* Modals */}
      {modal === 'detail' && (
        <DetailModal plant={selected} onEdit={p => { setSelected(p); setModal('form'); }} onClose={() => { setModal(null); setSelected(null); }} />
      )}
      {modal === 'form' && (
        <FormModal plant={selected} onSave={handleSave} onClose={() => { setModal(null); setSelected(null); }} />
      )}
      {modal === 'delete' && (
        <DeleteModal plant={selected} onConfirm={handleDelete} onCancel={() => { setModal(null); setSelected(null); }} />
      )}
    </div>
  );
}
